...
export async function deleteMarker(db: SQLiteDatabase, id: number): Promise<void> {
  try {
    ...
    // сначала явно чистим изображения, затем сам маркер (ON DELETE CASCADE подстрахует,
    // но явный порядок делает операцию предсказуемой и логируемой).
...
export async function getMarkers(db: SQLiteDatabase): Promise<Marker[]> {
  try {
    return await db.getAllAsync<Marker>(
      `${MARKERS_WITH_IMAGE_COUNT} GROUP BY m.id ORDER BY m.created_at DESC;`
    );
  } catch (error) {
...
export async function getMarkerById(db: SQLiteDatabase, id: number): Promise<Marker | null> {
  try {
    const marker = await db.getFirstAsync<Marker>(
      `${MARKERS_WITH_IMAGE_COUNT} WHERE m.id = ? GROUP BY m.id;`,
      id
    );
    return marker ?? null;
  } catch (error) {
