...
function checkProximity(coords: Location.LocationObjectCoords, currentMarkers: Marker[]) {
	...
	currentMarkers.forEach(marker => {
		const distance = calculateDistance(coords.latitude, coords.longitude, marker.latitude, marker.longitude);
...
export default function MapScreen() {
	...
	useEffect(() => {
		configureNotificationHandler()
			...
			.catch(err => {
				setNotificationsError(err instanceof Error ? err.message : 'Не удалось получить доступ к уведомлениям.');
			});
	}, []);
	...
	useEffect(() => {
		...
		const setupLocation = async () => {
			try {
				...
				if (cancelled) return;
				setLocationError(err instanceof Error ? err.message : 'Не удалось получить местоположение.');
			}
		};
	...
	if (error) {
		return (
			<View style={styles.centered}>
				<Text style={styles.errorText}>Не удалось инициализировать базу данных: {error.message}</Text>
		...
		<View style={styles.container}>
			...
			{(actionError || locationError || notificationsError) && (
				<View style={styles.bannerContainer}>
					...
					{notificationsError && <Text style={styles.actionErrorText}>{notificationsError}</Text>}
				</View>
			)}
