...
export class NotificationManager {
	...
	async pruneExcept(activeMarkerIds: Set<number>): Promise<void> {
		const toRemove = Array.from(this.activeNotifications.keys()).filter(markerId => !activeMarkerIds.has(markerId));
		await Promise.all(toRemove.map(markerId => this.removeNotification(markerId)));
	}

	async clearAll(): Promise<void> {
		await Promise.all(Array.from(this.activeNotifications.keys()).map(markerId => this.removeNotification(markerId)));
	}
}
